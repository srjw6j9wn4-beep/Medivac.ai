...
function newPassenger(): Passenger {
  return { id: crypto.randomUUID(), firstName: "", lastName: "", dob: "", weight: "", phone: "", notes: "" };
}
...
export default function PassengerManifest({ role }: Props) {
  ...
  function removeSector(id: string) {
    if (sectors.length > 1) setSectors(prev => prev.filter(s => s.id !== id));
  }

  // ── Passenger handlers ───────────────────────────────────────────────────────
  function updatePassenger(id: string, field: keyof Passenger, val: string) {
  ...
  async function saveManifest() {
    setSaving(true);
    try {
      const body = { flightDate, flightNumber, aircraftReg, bookingTeam, sectors, passengers, createdBy: (role as string) || "booking" };
      if (editingId) {
  ...
  function loadForEdit(m: any) {
    ...
    try { setPassengers(JSON.parse(m.passengers)); } catch { setPassengers([newPassenger()]); }
    setEditingId(m.id);
    setActiveTab("build");
  }
                ...
                {sectors.map((s, idx) => (
                  <div key={s.id} className="bg-slate-700/50 border border-slate-600 rounded-xl p-4">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      ...
                      />
                      <div className="flex flex-col gap-1">
                        <label className="text-xs font-medium text-slate-400 uppercase tracking-wide">ETD (local)</label>
                        <input type="time" value={s.etd} onChange={e => updateSector(s.id, "etd", e.target.value)}
                      ...
                      <div className="flex flex-col gap-1">
                        <label className="text-xs font-medium text-slate-400 uppercase tracking-wide">ETA (local)</label>
                        <input type="time" value={s.eta} onChange={e => updateSector(s.id, "eta", e.target.value)}
                          className="bg-slate-800 border border-slate-600 text-white text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
                      </div>
                    </div>
