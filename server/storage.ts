...
export type Invoice = { 
  id: number; invoiceNumber: string; invoiceDate: string; dueDate: string; serviceDate: string; 
  status: string; 
  ...
  payerType: string; payerName: string; taskRef: string | null; patientId: string | null; 
  pickupLocation: string | null; destination: string | null; aircraftReg: string | null; 
  missionType: string; baseAmount: number; afterHoursSurcharge: number; additionalCharges: number; 
  gstAmount: number; totalAmount: number; notes: string | null; submittedAt: string | null; 
  paidAt: string | null; createdAt: string; updatedAt: string; 
};
export type CharterQuote = { 
  id: number; quoteNumber: string; clientName: string; clientContact: string | null; purpose: string; 
  aircraftType: string; departureDate: string; legs: string; crew: string; costs: string; 
  totalCost: number; marginPercent: number; finalQuote: number; status: string; 
  autoInvoice: number;
  notes: string | null; createdAt: string; updatedAt: string; 
};
export type InsertCharterQuote = Omit<CharterQuote, 'id'>;
export type FuelReceipt = {
  id: number; receiptRef: string; entryMethod: string; aircraftReg: string; airportIcao: string;
  upliftDate: string; upliftLb: number; pricePerLb: number; totalAud: number; supplier: string;
  invoiceRef: string | null; scanImageUrl: string | null; reconStatus: string;
  reconBatchId: string | null; notes: string | null; enteredBy: string; createdAt: string; updatedAt: string;
};
...
function mapInvoice(r: any): Invoice {
  return {
    id: r.id, invoiceNumber: r.invoice_number, invoiceDate: r.invoice_date, dueDate: r.due_date,
    serviceDate: r.service_date, status: r.status,
    ...
    autoGenerated: r.auto_generated ?? 0, sourceType: r.source_type ?? null,
    payerType: r.payer_type, payerName: r.payer_name, taskRef: r.task_ref, patientId: r.patient_id,
    pickupLocation: r.pickup_location, destination: r.destination, aircraftReg: r.aircraft_reg,
    missionType: r.mission_type, baseAmount: r.base_amount, afterHoursSurcharge: r.after_hours_surcharge,
    additionalCharges: r.additional_charges, gstAmount: r.gst_amount, totalAmount: r.total_amount,
    notes: r.notes, submittedAt: r.submitted_at, paidAt: r.paid_at, createdAt: r.created_at, updatedAt: r.updated_at,
  };
}
function mapCharterQuote(r: any): CharterQuote {
  return { id: r.id, quoteNumber: r.quote_number, clientName: r.client_name, clientContact: r.client_contact, purpose: r.purpose, aircraftType: r.aircraft_type, departureDate: r.departure_date, legs: r.legs, crew: r.crew, costs: r.costs, totalCost: r.total_cost, marginPercent: r.margin_percent, finalQuote: r.final_quote, status: r.status, autoInvoice: r.auto_invoice ?? 0, notes: r.notes, createdAt: r.created_at, updatedAt: r.updated_at };
}
function mapFuelReceipt(r: any): FuelReceipt {
  return { id: r.id, receiptRef: r.receipt_ref, entryMethod: r.entry_method, aircraftReg: r.aircraft_reg, airportIcao: r.airport_icao, upliftDate: r.uplift_date, upliftLb: r.uplift_lb, pricePerLb: r.price_per_lb, totalAud: r.total_aud, supplier: r.supplier, invoiceRef: r.invoice_ref, scanImageUrl: r.scan_image_url, reconStatus: r.recon_status, reconBatchId: r.recon_batch_id, notes: r.notes, enteredBy: r.entered_by, createdAt: r.created_at, updatedAt: r.updated_at };
}
...
class DatabaseStorage {
  ...
  async createInvoice(d: Omit<Invoice, 'id'>): Promise<Invoice> {
    const row = { invoice_number: d.invoiceNumber, invoice_date: d.invoiceDate, due_date: d.dueDate, service_date: d.serviceDate, status: d.status, approval_status: d.approvalStatus ?? null, approved_by: d.approvedBy ?? null, approved_at: d.approvedAt ?? null, rejected_by: d.rejectedBy ?? null, rejected_at: d.rejectedAt ?? null, approval_note: d.approvalNote ?? null, auto_generated: d.autoGenerated ?? 0, source_type: d.sourceType ?? null, payer_type: d.payerType, payer_name: d.payerName, task_ref: d.taskRef, patient_id: d.patientId, pickup_location: d.pickupLocation, destination: d.destination, aircraft_reg: d.aircraftReg, mission_type: d.missionType, base_amount: d.baseAmount, after_hours_surcharge: d.afterHoursSurcharge, additional_charges: d.additionalCharges, gst_amount: d.gstAmount, total_amount: d.totalAmount, notes: d.notes, submitted_at: d.submittedAt, paid_at: d.paidAt, created_at: d.createdAt, updated_at: d.updatedAt };
  ...
  // -- Fuel Receipts ----------------------------------------------------------
  async listFuelReceipts(): Promise<FuelReceipt[]> {
    const { data } = await supabase.from('fuel_receipts').select('*').order('uplift_date', { ascending: false });
    return (data ?? []).map(mapFuelReceipt);
  }
  async getFuelReceipt(id: number): Promise<FuelReceipt | undefined> {
    const { data } = await supabase.from('fuel_receipts').select('*').eq('id', id).single();
    return data ? mapFuelReceipt(data) : undefined;
  }
  async createFuelReceipt(d: Omit<FuelReceipt, 'id'>): Promise<FuelReceipt> {
    const now = new Date().toISOString();
    const row = { receipt_ref: d.receiptRef, entry_method: d.entryMethod ?? 'manual', aircraft_reg: d.aircraftReg, airport_icao: d.airportIcao, uplift_date: d.upliftDate, uplift_lb: d.upliftLb, price_per_lb: d.pricePerLb, total_aud: d.totalAud, supplier: d.supplier, invoice_ref: d.invoiceRef ?? null, scan_image_url: d.scanImageUrl ?? null, recon_status: d.reconStatus ?? 'pending', recon_batch_id: d.reconBatchId ?? null, notes: d.notes ?? null, entered_by: d.enteredBy ?? 'ops', created_at: d.createdAt ?? now, updated_at: d.updatedAt ?? now };
    const { data } = await supabase.from('fuel_receipts').insert(row).select().single();
    return mapFuelReceipt(data);
  }
  async updateFuelReceipt(id: number, u: Partial<FuelReceipt>): Promise<FuelReceipt | undefined> {
    const row: any = { updated_at: new Date().toISOString() };
    const map: Record<string, string> = { receiptRef: 'receipt_ref', entryMethod: 'entry_method', aircraftReg: 'aircraft_reg', airportIcao: 'airport_icao', upliftDate: 'uplift_date', upliftLb: 'uplift_lb', pricePerLb: 'price_per_lb', totalAud: 'total_aud', supplier: 'supplier', invoiceRef: 'invoice_ref', scanImageUrl: 'scan_image_url', reconStatus: 'recon_status', reconBatchId: 'recon_batch_id', notes: 'notes', enteredBy: 'entered_by' };
    for (const [k, v] of Object.entries(map)) if ((u as any)[k] !== undefined) row[v] = (u as any)[k];
    const { data } = await supabase.from('fuel_receipts').update(row).eq('id', id).select().single();
    return data ? mapFuelReceipt(data) : undefined;
  }
  async deleteFuelReceipt(id: number): Promise<boolean> {
    const { error } = await supabase.from('fuel_receipts').delete().eq('id', id);
    return !error;
  }

  // -- Auto-Invoice Generation --------------------------------------------------
  async autoGenerateInvoice(params: {
    sourceType: 'nept' | 'charter' | 'accommodation' | 'dental' | 'special';
    taskRef: string | null;
    serviceDate: string;
    ...
    const now = new Date().toISOString();
    const prefixMap: Record<string, string> = { nept: 'NEPT', charter: 'CHT', accommodation: 'ACC', dental: 'DENT', special: 'SPEC' };
    const prefix = prefixMap[params.sourceType] ?? 'INV';
    const year = new Date().getFullYear();
    const { count } = await supabase.from('invoices').select('*', { count: 'exact', head: true }).like('invoice_number', `%${prefix}-${year}%`).then(r => ({ count: r.count ?? 0 }));
    const invoiceNumber = `INV-${prefix}-${year}-${String((count ?? 0) + 1).padStart(4, '0')}`;

    const baseAmount = params.baseAmountCents / 100;
    const afterHoursSurcharge = (params.afterHoursSurchargeCents ?? 0) / 100;
    const gstAmount = params.gstApplicable ? (baseAmount + afterHoursSurcharge) * 0.1 : 0;
    const totalAmount = baseAmount + afterHoursSurcharge + gstAmount;
    ...
    await this.createNotification({
      ...
      body: `Auto-generated invoice ${invoiceNumber} for ${params.payerName} (${params.missionType}) requires approval.`,
      taskRef: params.taskRef ?? undefined,
    });

    await this.logAudit('invoice', String(invoice.id), 'auto_generated', 'system', `Auto-generated from ${params.sourceType} source, task ref ${params.taskRef ?? 'n/a'}`);
